...
- `mcp-ee-gh.yaml` - This is the main definition file that `ansible-builder` will use to construct your EE image.
...
cd mcp-ee-gh/mcp-ee-gh
```

Open `mcp-ee-gh.yaml` in your favorite text editor and review the configuration.
...
Before you build the EE, you must ensure `ansible-builder` can access all the collections you listed in your `mcp-ee-gh.yaml` file.
...
```bash
ansible-builder build --file mcp-ee-gh.yaml --tag mcp-ee-gh:latest --container-runtime podman
```

This command uses your `mcp-ee-gh.yaml` file to build an image and tags it as `mcp-ee-gh:latest`
...
```bash
ansible-navigator run playbook.yml --execution-environment-image mcp-ee-gh:latest
```
...
1. Tag your local image for the target registry (e.g., `quay.io/your-username/mcp-ee-gh:latest`).

```bash
podman tag mcp-ee-gh:latest quay.io/your-username/mcp-ee-gh:latest
```

2. Push the tagged image to the target registry:

```bash
podman push quay.io/your-username/mcp-ee-gh:latest
```
