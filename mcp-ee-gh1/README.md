...
- `mcp-ee-gh1.yaml` - This is the main definition file that `ansible-builder` will use to construct your EE image.
...
cd mcp-ee-gh/mcp-ee-gh1
```

Open `mcp-ee-gh1.yaml` in your favorite text editor and review the configuration.
...
Before you build the EE, you must ensure `ansible-builder` can access all the collections you listed in your `mcp-ee-gh1.yaml` file.
...
```bash
ansible-builder build --file mcp-ee-gh1.yaml --tag mcp-ee-gh1:latest --container-runtime podman
```

This command uses your `mcp-ee-gh1.yaml` file to build an image and tags it as `mcp-ee-gh1:latest`
...
```bash
ansible-navigator run playbook.yml --execution-environment-image mcp-ee-gh1:latest
```
...
1. Tag your local image for the target registry (e.g., `quay.io/your-username/mcp-ee-gh1:latest`).

```bash
podman tag mcp-ee-gh1:latest quay.io/your-username/mcp-ee-gh1:latest
```

2. Push the tagged image to the target registry:

```bash
podman push quay.io/your-username/mcp-ee-gh1:latest
```
